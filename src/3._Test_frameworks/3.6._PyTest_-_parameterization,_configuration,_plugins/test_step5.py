...
def test_pieces_of_the_message(browser, slug):
    ...
    browser.find_element(By.CSS_SELECTOR, 'textarea').send_keys(str(answer))
    button3 = browser.find_element(By.CSS_SELECTOR, 'button[class="submit-submission"]').click()
    print('ELEMENT WAS FOUND!')
    
    # BLOCK WITH CORRECT OR NOT CORRECT TEXT:
    correct_text = browser.find_element(By.CSS_SELECTOR, 'p[class="smart-hints__hint"]').text
    ...
    if correct_text == 'Correct!':
        ...
        time.sleep(30)
        browser.get(link)
        button4 = browser.find_element(By.CSS_SELECTOR, 'button[class="again-btn white"]').click()
        button5 = browser.find_elements(By.CSS_SELECTOR, 'button[type="button"]').text
